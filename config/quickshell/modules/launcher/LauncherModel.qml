...
Item {
    ...
    readonly property var results: {
        ...
        if (mode === "all") {
            const switchers = [
                {
                    title: "Emoji Picker",
                    subtitle: "search + paste emoji",
                    icon: "\u{1f600}",
                    go: "emoji"
                },
                {
                    title: "Nerd Fonts",
                    subtitle: "search + paste icons",
                    icon: "\u{f0b10}",
                    go: "nerd"
                },
                {
                    title: "Clipboard History",
                    subtitle: "paste recent clips",
                    icon: "\u{f0147}",
                    go: "clipboard"
                },
                {
                ...
                {
                    title: "Wallpapers",
                    subtitle: "preview and choose",
                    icon: "\u{f108}",
                    go: "wallpaper"
                },
                {
                    title: "Power",
                    subtitle: "lock, reboot, shutdown\u2026",
                    icon: "\u{f0425}",
                    go: "power"
                }
            ];
        ...
        if (mode === "emoji") {
            results = results.concat(emojiSvc.emojis.map(entry => ({
                        kind: "emoji",
                        title: entry.e + "  " + entry.n,
                        subtitle: entry.n,
                        icon: entry.e,
                        text: entry.e,
                        actionHint: "\u21b5 paste",
                        _score: score(entry.n, query) * 0.8
                    })).filter(item => item._score > 0));
        }
        if (mode === "nerd") {
            results = results.concat(nerdSvc.icons.map(entry => ({
                        kind: "nerd",
                        title: entry.c + "  " + entry.n,
                        subtitle: entry.k + " \u00b7 " + entry.n,
                        icon: entry.c,
                        text: entry.c,
                        actionHint: "\u21b5 paste",
                        _score: score(entry.n, query) * 0.7
                    })).filter(item => item._score > 0));
        }
        if (mode === "bluetooth") {
            ...
            if (query)
                devices = devices.filter(item => item._score > 0);
            devices.sort((left, right) => (right.connected ? 1 : 0) - (left.connected ? 1 : 0));
            if (!query)
                devices.unshift({
                    kind: "bt",
                    title: btSvc.powered ? "Bluetooth On" : "Bluetooth Off",
    ...
    Services.EmojiService {
        id: emojiSvc
    }
    Services.NerdFontService {
        id: nerdSvc
    }
    ...
    function triggerSelected(ctrl) {
        ...
        if (item.kind === "app") {
            ...
            root.query = "";
            root.selected = 0;
            if (item.go === "bluetooth")
                btSvc.refresh();
            root.sourceOpened();
        } else if (item.kind === "bt") {
            if (item._action === "power")
                btSvc.togglePower();
            else if (item.connected)
                btSvc.disconnect(item.addr);
            else
