...
Item {
    ...
    property int notifCount: {
        ...
        if (c !== undefined && c !== null)
            return c;
        if (server.trackedNotifications.values)
            return server.trackedNotifications.values.length;
        if (server.trackedNotifications.length !== undefined)
            return server.trackedNotifications.length;
        return 0;
    }
    ...
    function dismissAll() {
        ...
        while (notifCount > 0 && guard-- > 0) {
            try {
                let n = null;
                if (server.trackedNotifications.get)
                    n = server.trackedNotifications.get(0);
                else if (server.trackedNotifications.values)
                    n = server.trackedNotifications.values[0];
                else
                    break;
                if (!n)
                    break;
    ...
    function grouped() {
        ...
        if (!src || src.length === 0) {
            ...
            for (let i = 0; i < cnt; i++) {
                try {
                    let n = server.trackedNotifications.get(i);
                    if (n)
                        src.push(n);
                } catch (e) {}
            }
        ...
        for (let n of src) {
            ...
            if (n.appIcon)
                map[key].appIcon = n.appIcon;
        }
        return order.map(k => map[k]);
    }

    function dismissGroup(group) {
        for (let n of group.notifications)
            try {
                n.dismiss();
            } catch (e) {}
    }
