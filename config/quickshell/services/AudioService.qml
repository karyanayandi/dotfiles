...
Item {
    ...
    function showOsd(kind) {
        ...
        var m = svc.muted
        var v = svc.vol
        if (svc.sink && svc.sink.audio && svc.sink.audio.muted) m = true
        svc.osdPercent = Math.round(v * 100)
        if (kind === "mic") svc.osdIcon = m ? "" : ""
        else {
            if (m) svc.osdIcon = "󰸈"
            else if (v <= 0.01) svc.osdIcon = ""
            else if (v < 0.34) svc.osdIcon = ""
            else if (v < 0.67) svc.osdIcon = "󰕾"
            else svc.osdIcon = ""
        }
        svc.osdVisible = true
        osdHide.restart()
    ...
    Timer { interval: 200; running: true; repeat: true; onTriggered: volPoll.running = true; Component.onCompleted: volPoll.running = true }
