...
Item {
    ...
    Component.onCompleted: {
        ...
        try {
            ...
            if (Array.isArray(v))
                root.history = v.slice().sort((a, b) => (b.time || 0) - (a.time || 0));
        } catch (e) {}
        pollTimer.start();
    }
        ...
        stdout: SplitParser {
            onRead: data => {
                ...
                if (hash === root._lastHash)
                    return;
                root._lastHash = hash;
                if (root.history.length && (isImg ? root.history[0].img === t : root.history[0].text === t))
                    return;
                let idx = root.history.findIndex(x => isImg ? x.img === t : x.text === t);
                if (idx !== -1)
                    root.history.splice(idx, 1);
                let entry = isImg ? {
                    img: t,
                    text: "",
                    time: Date.now(),
                    preview: "\u{1f5bc} Image"
                } : {
                    text: t,
                    time: Date.now(),
                ...
                if (root.history.length > root.maxSize)
                    root.history = root.history.slice(0, root.maxSize);
                root.historyChanged();
                root.save();
            }
        }
