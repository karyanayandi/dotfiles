...
export async function openSettingsMenu(
  ...
  const showSettings = () =>
    ui.custom((_tui, theme, _kb, done) => {
      ...
      const list = new SettingsList(
        ...
        /* onChange */ (id, newValue) => {
          ...
          if (id === "autoClearCompleted") {
            cfg.autoClearCompleted =
              newValue as TasksConfig["autoClearCompleted"]
            saveTasksConfig(cfg)
          }
          if (id === "showAll") {
      ...
      root.addChild(
        new Text(theme.bold(theme.fg("accent", "⚙  Task Settings")), 0, 0),
      )
  ...
  const exit = await Effect.runPromiseExit(Effect.tryPromise(showSettings))
