/**
 * tasks — A pi extension providing Claude Code-style task tracking and coordination.
 *
 * Tools:
 ...
 *   TaskExecute  — Execute tasks as subagents (requires subagents extension)
 *
 * Commands:
...
import { AutoClearManager } from "./src/auto-clear.ts"
import { ProcessTracker } from "./src/process-tracker.ts"
import {
  type CadenceConfig,
  createCadenceState,
  ...
  onTurnStart,
  resetCadenceState,
} from "./src/reminder-cadence.ts"
import { TaskStore } from "./src/task-store.ts"
import { loadTasksConfig } from "./src/tasks-config.ts"
import type { Task } from "./src/types.ts"
import { openSettingsMenu } from "./src/ui/settings-menu.ts"
import { TaskWidget, type UICtx } from "./src/ui/task-widget.ts"

// ---- Debug ----

const DEBUG = !!process.env.PI_TASKS_DEBUG
function debug(...args: unknown[]) {
  if (DEBUG) console.error("[tasks]", ...args)
}
...
/**
 * Build the system reminder, shaped after Claude Code's todo reminders.
 */
...
export default function tasks(pi: ExtensionAPI) {
  ...
  let cascadeConfig:
    | { additionalContext?: string; model?: string; maxTurns?: number }
    | undefined
  ...
  function rpcCall<T>(
    ...
    return Effect.runPromise(
      Effect.tryPromise({
        try: () =>
          new Promise<T>((resolve, reject) => {
            ...
            const timer = setTimeout(() => {
              ...
              reject(new TasksError(`${channel} timeout`))
            }, timeoutMs)
            ...
            const unsub = pi.events.on(
              ...
              (raw: unknown) => {
                ...
                else reject(new TasksError(reply.error))
              },
            )
        ...
        catch: (e) => new TasksError(String(e)),
      }),
    )
  }
    ...
    const unsub = pi.events.on(
      ...
      (raw: unknown) => {
        ...
        if (remoteVersion === undefined) {
          pendingWarning =
            "pi-subagents extension is outdated — please update for task execution support."
        } else if (remoteVersion > PROTOCOL_VERSION) {
          pendingWarning = `pi-tasks is outdated (protocol v${PROTOCOL_VERSION}, pi-subagents has v${remoteVersion}) — please update for task execution support.`
        } else if (remoteVersion < PROTOCOL_VERSION) {
          pendingWarning = `pi-subagents extension is outdated (protocol v${remoteVersion}, pi-tasks has v${PROTOCOL_VERSION}) — please update for task execution support.`
        } else {
    ...
    if (task.blockedBy && task.blockedBy.length > 0) {
      ...
      for (const depId of task.blockedBy) {
        ...
        if (dep?.metadata?.result) {
          const result =
            dep.metadata.result.length > 4000
      ...
      const unblocked = store
        ...
        .filter(
          (t) =>
            ...
            t.blockedBy.every(
              (depId) => store.get(depId)?.status === "completed",
            ),
        )
      for (const next of unblocked) {
        ...
        try {
          ...
          store.update(next.id, {
            owner: agentId,
            metadata: { ...next.metadata, agentId },
          })
  ...
  pi.events.on("subagents:failed", (data) => {
    const { id, error, result, status } = data as {
      id: string
      error?: string
      result?: string
      status: string
  ...
  pi.on("turn_end", async (event) => {
    ...
    if (!cadence.reminderInjectedThisCycle && !cadence.reminderDue) {
      ...
      if (
        gap >= ACTIVE_REMINDER_INTERVAL &&
        store.list().some((t) => t.status === "in_progress")
      ) {
  ...
  pi.on("tool_result", async (event) => {
    ...
    if (
      cadence.currentTurn - cadence.lastTaskToolUseTurn <
      ACTIVE_REMINDER_INTERVAL
    )
      return {}
    ...
    return {
      messages: [
        ...event.messages,
        {
          role: "user" as const,
          content: [
            { type: "text" as const, text: buildSystemReminder(tasks) },
          ],
          timestamp: Date.now(),
        },
      ],
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    showPersistedTasks(
      reason === "reload" || reason === "resume" || reason === "fork",
    )
...
- Include \`agentType\` (e.g., "general-purpose", "Explore") to mark tasks for subagent execution via TaskExecute`,
    ...
    parameters: Type.Object({
      ...
      agentType: Type.Optional(
        Type.String({
          description:
            "Agent type for subagent execution (e.g., 'general-purpose', 'Explore')",
        }),
      ),
...
- Works with all task types: background shells, async agents, and remote sessions`,
    ...
    async execute(_toolCallId, params, signal, _onUpdate, _ctx) {
      ...
      if (!processOutput) {
        ...
        if (!task) throw new TasksError(`No task found with ID ${task_id}`)
        ...
        if (task.metadata?.agentId) {
              ...
              Effect.tryPromise({
                try: () =>
                  new Promise<void>((resolve) => {
                    ...
                    const unsubOk = pi.events.on(
                      "subagents:completed",
                      (d: unknown) => {
                        if ((d as any).id === task.metadata?.agentId) {
                          unsubOk()
                          unsubFail()
                          cleanup()
                        }
          ...
          return textResult(
            `Task #${task_id} [${updated.status}] — subagent ${task.metadata.agentId}`,
          )
        }
        throw new TasksError(`No background process for task ${task_id}`)
      }
...
- Use this tool when you need to terminate a long-running task`,
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      if (!taskId) throw new TasksError("task_id is required")
      ...
      if (!stopped) {
        ...
        throw new TasksError(`No running background process for task ${taskId}`)
      }
...
- **max_turns**: Maximum turns per agent`,
    ...
    parameters: Type.Object({
      task_ids: Type.Array(Type.String(), {
        description: "Task IDs to execute as subagents",
      }),
      additional_context: Type.Optional(
        Type.String({ description: "Extra context for agent prompts" }),
      ),
      model: Type.Optional(
        Type.String({ description: "Model override for agents" }),
      ),
      max_turns: Type.Optional(
        Type.Number({ description: "Max turns per agent", minimum: 1 }),
      ),
    }),
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      for (const taskId of params.task_ids) {
        ...
        if (openBlockers.length > 0) {
          results.push(
            `#${taskId}: blocked by ${openBlockers.map((id) => "#" + id).join(", ")}`,
          )
          continue
        }
  ...
  pi.registerCommand("tasks", {
    ...
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      ...
      const runMenu = Effect.tryPromise(() => mainMenu())
      const exit = await Effect.runPromiseExit(runMenu)
      if (Exit.isFailure(exit) && !Cause.hasInterruptsOnly(exit.cause)) {
        const [first] = Cause.prettyErrors(exit.cause)
        throw new TasksError(first?.message ?? Cause.pretty(exit.cause))
      }
    },
  })
}
